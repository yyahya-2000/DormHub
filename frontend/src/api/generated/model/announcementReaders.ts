 ...
 */
import type { AnnouncementReader } from './announcementReader';

export interface AnnouncementReaders {
  announcement_id: number;
  title?: string;
  is_mandatory?: boolean;
  /** The dormitories the figures below are about. For an announcement addressed to every dormitory this is the caller's own scope and not the whole register; for the administrator it is empty, meaning all of them. */
  building_ids?: number[];
  /** The residents of those dormitories by the register, less those it says have moved out (FR-05). */
  audience_size: number;
  acknowledged_count: number;
  /**
     * The acknowledged fraction of the audience. An empty audience gives 1, not 0: a share of zero over zero people would put a red figure in front of a warden with nobody to chase.
     * @minimum 0
     * @maximum 1
     */
  acknowledged_share: number;
  acknowledged: AnnouncementReader[];
  /** FR-12's named list, which is why the acknowledgement is a row and not a counter. */
  not_acknowledged: AnnouncementReader[];
}
