 ...
 */
import type { GuestDocumentType } from './guestDocumentType';
...
export interface GuestRequest {
  ...
  guest_doc_type?: GuestDocumentType;
  guest_doc_type_label?: string;
  /** The last four characters, everything before them hidden (NFR-06). The number itself is on an audited route of its own. */
  guest_doc_number_masked?: string;
  /** Derived from the document type, never accepted from the client. */
  is_foreign_document?: boolean;
  /** FR-23, first criterion. Attached by the server when the document is a foreign one, null otherwise. The text is configuration, because the procedure is the university's and §2.7.4 leaves open which of two deadlines under art. 20 of Federal Law No. 109-FZ applies. */
  foreign_guest_warning?: string | null;
  purpose?: string | null;
  visit_date?: string;
  planned_from?: string;
  planned_to?: string;
  /** True when the interval runs past midnight, which is only possible where the dormitory's own regime admits it. FR-23's second criterion turns on this. */
  spans_more_than_one_day?: boolean;
  /** The departure deadline: the earlier of the end of the interval and the building's control time. */
  due_at?: string;
  ...
  responsible_officer_mark?: string | null;
  responsible_officer_mark_at?: string | null;
