...
/**
 * The refusal of an operation that would process personal data with no consent on record. 409 rather than 403: the caller's role covers the object, and what stands in the way is a missing document. The first route to answer with it is the security post's `check-in`, in increment 1.
 */
export interface ConsentRequired {
  message: string;
  document: ConsentDocument;
  /** The revision in force, so the client knows which text to put on the screen. */
  revision: string;
}
